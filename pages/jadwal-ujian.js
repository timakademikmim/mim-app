(function () {
  ...
  function buildExamScanWorkspaceHtml(row, participants, questionRows, uploadsByClass) {
    var grouped = groupParticipantsByClass(participants)
    if (!participants.length) return '<div class="placeholder-card">Peserta ujian belum dirilis untuk mapel ini.</div>'
    ...
    grouped.forEach(function (items, kelasId) {
      var first = items[0] || {}
      var soalRow = pickQuestionRowForClass(questionRows, first.kelas_nama || '')
      var answerKey = extractMultipleChoiceAnswerKey(soalRow)
      var pgCount = countMultipleChoiceQuestions(soalRow)
      var upload = uploadsByClass[String(kelasId || '')] || null
      var keyHtml = answerKey.length
        ? answerKey.map(function (item) {
            return '<span style="display:inline-flex; align-items:center; justify-content:center; min-width:42px; padding:6px 8px; border-radius:999px; background:#eff6ff; color:#1d4ed8; font-weight:700; font-size:12px;">' + item.no + item.answer + '</span>'
          }).join('')
        : '<span style="font-size:12px; color:#b45309;">Kunci jawaban PG belum lengkap.</span>'

      html += '<div style="border:1px solid #dbeafe; border-radius:14px; background:linear-gradient(180deg,#f8fbff 0%,#ffffff 100%); padding:14px;">'
      html += '<div style="display:flex; align-items:flex-start; justify-content:space-between; gap:12px; flex-wrap:wrap; margin-bottom:10px;">'
      html += '<div><div style="font-weight:800; color:#0f172a; font-size:15px;">Kelas ' + escapeHtml(first.kelas_nama || '-') + '</div><div style="font-size:12px; color:#475569; margin-top:4px;">Kode ujian: <span style="font-family:monospace; font-weight:700;">' + escapeHtml(first.exam_code || '-') + '</span> | Peserta: ' + items.length + ' santri | PG: ' + pgCount + ' nomor</div></div>'
      html += '<div style="display:flex; gap:8px; flex-wrap:wrap;">'
      html += '<label class="ju-btn secondary" style="cursor:pointer; margin:0;">Upload Foto/Scan<input type="file" accept="image/*" capture="environment" style="display:none;" onchange="onExamScanWorkspaceUploadChange(\'' + escapeHtml(String(kelasId || '')) + '\', this)"></label>'
      html += '<button type="button" class="ju-btn secondary" onclick="clearExamScanWorkspaceUpload(\'' + escapeHtml(String(kelasId || '')) + '\')"' + (upload ? '' : ' disabled') + '>Hapus Preview</button>'
      html += '</div>'
      html += '</div>'
      html += '<div style="display:grid; grid-template-columns:minmax(0,1fr) minmax(280px,360px); gap:12px; align-items:start;">'
      html += '<div style="border:1px solid #e2e8f0; border-radius:12px; background:#fff; padding:12px;">'
      html += '<div style="font-weight:700; color:#0f172a; margin-bottom:8px;">Kunci Jawaban PG</div>'
      html += '<div style="display:flex; flex-wrap:wrap; gap:8px; max-height:180px; overflow:auto;">' + keyHtml + '</div>'
      html += '<div style="margin-top:10px; font-size:12px; color:#64748b;">Workspace ini dipakai untuk tahap scan PG. Upload foto lembar jawaban per kelas di sini agar langkah deteksi otomatis berikutnya punya tempat kerja yang tetap.</div>'
      html += '</div>'
      html += '<div style="border:1px dashed #cbd5e1; border-radius:12px; background:#fff; min-height:220px; padding:10px;">'
      if (upload && upload.dataUrl) {
        html += '<div style="font-size:12px; color:#475569; margin-bottom:8px;">Preview: ' + escapeHtml(upload.fileName || 'Gambar jawaban') + '</div>'
        html += '<img src="' + escapeHtml(upload.dataUrl) + '" alt="Preview scan ' + escapeHtml(first.kelas_nama || '-') + '" style="display:block; width:100%; max-height:420px; object-fit:contain; border-radius:10px; background:#f8fafc;">'
      } else {
        html += '<div style="height:100%; min-height:200px; display:flex; align-items:center; justify-content:center; text-align:center; color:#64748b; font-size:13px;">Belum ada foto/scan yang diupload untuk kelas ini.</div>'
      }
      html += '</div>'
      html += '</div>'
      html += '</div>'
  ...
  function renderExamScanWorkspaceBody() {
    var bodyEl = getInput('ju-scan-workspace-modal-body')
    if (!bodyEl) return
    bodyEl.innerHTML = buildExamScanWorkspaceHtml(
      activeScanWorkspaceState.row,
      activeScanWorkspaceState.participants || [],
      activeScanWorkspaceState.questionRows || [],
      activeScanWorkspaceState.uploadsByClass || {}
    )
  }

  window.openExamScanWorkspaceModal = async function openExamScanWorkspaceModal() {
    ...
    if (!row || !participants.length) {
      alert('Buka data peserta ujian terlebih dahulu.')
      return
    }

    var modalEl = getInput('ju-scan-workspace-modal')
    var titleEl = getInput('ju-scan-workspace-modal-title')
    var bodyEl = getInput('ju-scan-workspace-modal-body')
    if (!modalEl || !titleEl || !bodyEl) return

    modalEl.style.display = 'flex'
    titleEl.textContent = 'Workspace Scan PG: ' + String(row.mapel || '-')
    bodyEl.innerHTML = 'Loading workspace scan...'
    ...
    activeScanWorkspaceState = {
      row: row,
      participants: participants.slice(),
      questionRows: questionRes.rows || [],
      uploadsByClass: activeScanWorkspaceState.uploadsByClass || {}
    }
    renderExamScanWorkspaceBody()
  }

  window.closeExamScanWorkspaceModal = function closeExamScanWorkspaceModal(event) {
    if (event && event.target && event.target.id !== 'ju-scan-workspace-modal') return
    var modalEl = getInput('ju-scan-workspace-modal')
    if (modalEl) modalEl.style.display = 'none'
  }

  window.onExamScanWorkspaceUploadChange = function onExamScanWorkspaceUploadChange(kelasId, inputEl) {
    ...
    reader.onload = function () {
      activeScanWorkspaceState.uploadsByClass[String(kelasId || '')] = {
  ...
  window.clearExamScanWorkspaceUpload = function clearExamScanWorkspaceUpload(kelasId) {
    var key = String(kelasId || '')
    if (activeScanWorkspaceState.uploadsByClass && activeScanWorkspaceState.uploadsByClass[key]) {
      delete activeScanWorkspaceState.uploadsByClass[key]
      renderExamScanWorkspaceBody()
    }
  }
