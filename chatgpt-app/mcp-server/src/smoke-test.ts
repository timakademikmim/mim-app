...
function requiredEnv(name: string, fallback = "") {
  ...
  if (!value) throw new Error(`${name} wajib diisi untuk smoke test`)
  return value
}
...
async function run() {
  ...
  if (externalSubject) {
    ...
    if (String((resolveResult as Record<string, unknown>)?.status || "") === "linked") {
      const guru = ((resolveResult as Record<string, unknown>)?.guru || {}) as Record<string, unknown>
  ...
  if (String((targetResult as Record<string, unknown>)?.status || "") !== "found") {
    console.log("Smoke test berhenti: target ujian belum tunggal.")
    return
  }

  const target = ((targetResult as Record<string, unknown>)?.target || {}) as Record<string, unknown>
