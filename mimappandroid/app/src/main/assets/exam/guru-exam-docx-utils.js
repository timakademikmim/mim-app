;(function initGuruExamDocxUtils() {
  ...
  function setParagraphSpacing(paragraph, opts = {}) {
    if (!paragraph) return
    const pPr = ensureParagraphProperties(paragraph)
    let spacing = first(pPr, 'spacing', true)
    if (!spacing) {
      spacing = createWordNode(paragraph.ownerDocument, 'spacing')
      pPr.appendChild(spacing)
    }
    spacing.setAttributeNS(WORD_NS, 'w:before', String(Number(opts.before ?? 0)))
    spacing.setAttributeNS(WORD_NS, 'w:after', String(Number(opts.after ?? 0)))
    spacing.setAttributeNS(WORD_NS, 'w:line', String(Number(opts.line ?? 220)))
    spacing.setAttributeNS(WORD_NS, 'w:lineRule', String(opts.lineRule || 'auto'))
  }

  function compactCellParagraphs(cell, opts = {}) {
    qsa(cell, 'p', true).forEach(paragraph => {
      removeParagraphIndent(paragraph)
      setParagraphSpacing(paragraph, opts)
    })
  }

  function compactRow(row, opts = {}) {
    if (!row) return row
    const trPr = first(row, 'trPr', true)
    const trHeight = trPr ? first(trPr, 'trHeight', true) : null
    if (trPr && trHeight) trPr.removeChild(trHeight)
    getRowCells(row).forEach(cell => compactCellParagraphs(cell, opts))
    return row
  }
  ...
  function fillCrosswordTable(table, section, sectionIndex, templates, headerMeta = {}) {
    ...
    if (splitGridTemplate) {
      ...
      const infoLines = []
      const infoText = resolveSectionInfoText(section, headerMeta)
      if (infoText) infoLines.push(...splitPreserveLines(infoText))
      if (singleItem) {
      ...
      const clueLines = singleItem ? buildCrosswordClueLines(singleItem, headerMeta.lang || 'AR') : []
      let afterGridTable = null
      if (templates.info && clueLines.length) {
        afterGridTable = templates.table.cloneNode(true)
        clearTableRows(afterGridTable)
        afterGridTable.appendChild(fillSectionInfoRow(cloneRow(templates.info), clueLines))
      }
      return { gridTable: outputGridTable, afterGridTable }
    }
    ...
    if (compactTemplate && singleItem) {
      ...
      compactClueLines = buildCrosswordClueLines(singleItem, headerMeta.lang || 'AR')
    }
    if (templates.info && infoLines.some(line => String(line || '').trim())) {
      table.appendChild(fillSectionInfoRow(cloneRow(templates.info), infoLines))
    }
    ...
    if (compactTemplate && templates.info && compactClueLines.length) {
      table.appendChild(fillSectionInfoRow(cloneRow(templates.info), compactClueLines))
    }
    return null
  }
  ...
  function fillFillBlankTable(table, section, sectionIndex, templates, headerMeta = {}) {
    ...
    const infoText = resolveSectionInfoText(section, headerMeta)
    if (templates.info && infoText) {
      table.appendChild(compactRow(fillSectionInfoRow(cloneRow(templates.info), infoText), { after: 0, line: 200 }))
    }
    ...
    table.appendChild(compactRow(bankRow, { after: 0, line: 200 }))
    ...
    ;(section.items || []).forEach((item, idx) => {
      ...
      if (cells[2]) setCellText(cells[2], '.')
      if (cells[3]) setCellTextWithImages(cells[3], String(item?.text || ''), item?.images || item?.imageUrl || item?.image_url || '')
      table.appendChild(compactRow(questionRow, { after: 0, line: 200 }))
    })
  }
  ...
  function rebuildBody(doc, sections, headerMeta = {}) {
    ...
    if (generalInstruction) {
      ...
      if (infoCells[0]) {
        if (isArabic) {
          setCellLines(infoCells[0], instructionLines)
        } else {
      ...
      if (infoCells[1]) {
        if (isArabic) {
          ...
          setCellLines(infoCells[1], instructionLines)
        }
      }
    ...
    activeSections.forEach((section, sectionIndex) => {
      ...
      if (extra?.afterGridTable) body.insertBefore(extra.afterGridTable, sectionPr)
      if (spacerParagraph) body.insertBefore(spacerParagraph.cloneNode(true), sectionPr)
    })
  }
  ...
  function rebuildBodyIndonesian(doc, sections, headerMeta = {}) {
    ...
    if (generalInstruction.trim()) {
      ...
      const infoCells = getRowCells(infoRow)
      if (infoCells[0]) {
        setCellLines(infoCells[0], splitPreserveLines(generalInstruction))
      }
      generalTable.appendChild(infoRow)
      body.insertBefore(generalTable, sectionPr)
      if (spacerParagraph) body.insertBefore(spacerParagraph.cloneNode(true), sectionPr)
  ...
  function transformExamDocumentXml(templateXml, soal = {}, headerMeta = {}) {
    const payload = typeof soal?.questions_json === 'string'
      ? JSON.parse(soal.questions_json || '[]')
      : (Array.isArray(soal?.questions_json) ? soal.questions_json : [])
    const questions = Array.isArray(payload?.questions) ? payload.questions : (Array.isArray(payload) ? payload : [])
    ...
    const sections = buildSectionsFromQuestions(questions, fallbackType)
    if (!sections.length) {
      throw new Error('Bagian soal tidak berhasil dibentuk.')
    }
    const xmlString = String(templateXml || '').trim()
    if (!xmlString) {
      throw new Error('Template DOCX tidak memiliki XML dokumen.')
    }
    const doc = createXmlDocument(xmlString)
    const lang = String(headerMeta?.lang || parseInstructionMeta(soal?.instruksi, 'ID')?.lang || 'ID').toUpperCase() === 'AR'
      ? 'AR'
      : 'ID'
    if (lang === 'AR') {
      rebuildBody(doc, sections, { ...headerMeta, lang: 'AR' })
